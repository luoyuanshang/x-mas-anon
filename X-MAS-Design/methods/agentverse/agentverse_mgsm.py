...
class Agentverse_MGSM(MAS):
    ...
    def inference(self, query):
        print("Starting the Agentverse_HumanEval pipeline")
        for i in range(self.max_turn):
            ...
            print(f"Final Solution Evaluation: {feedback} with output {Score}")
            if Score == 1:
                break
            else:
    ...
    def assign_roles(self, query: str):
        ...
        role_assignment_response = self.call_llm(None, None, assigner_messages,model_name=self.model_names[0])
        print(f"[Assigner]: {role_assignment_response}")
    ...
    def group_vertical_solver_first(self, query: str, role_descriptions: List[str]):
        ...
        solver_response = self.call_llm(None, None, solver_message, model_name=self.model_names[1])
        self.history.append(
            {
                "role": "assistant",
        ...
        for i in range(self.max_criticizing_rounds):
            ...
            for j in range(cnt_critic_agent):
                ...
                print(f"history_critic: {history_critic}")
                critic_message = self.construct_messages(critic_prepend_prompt, history_critic, critic_append_prompt)
                critic_response = self.call_llm(None, None, critic_message, model_name=self.model_names[2])
    ...
    def evaluate(self, query: str, role_descriptions: List[str], Plan):
        ...
        evaluator_response = self.call_llm(None, None, evaluator_message, model_name=self.model_names[3])
