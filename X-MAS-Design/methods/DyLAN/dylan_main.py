...
class DyLAN_Main(MAS):
    ...
    def __init__(self, general_config, method_config_name="config_main"):
        ...
        self.model_names = self.method_config.get("model_names", ["qwen-2.5-32b-instruct"] * self.num_agents)
    ...
    def inference(self, query):
        """
        Process a query through the DyLAN network.
        
        Args:
            query: The input query string
            
        Returns:
            A response string processed by DyLAN
        """
        ...
        return self._most_frequent([self.nodes[idx]['answer'] for idx in idxs], self.cmp_res)[0]
    ...
    def _activate_node(self, idx, question):
        ...
        node['reply'] = self.call_llm(messages=contexts, model_name=node['model_name'])
    ...
    def _listwise_ranker(self, responses, question):
        ...
        completion = self.call_llm(messages=[message], model_name=self.model_names[-1])
