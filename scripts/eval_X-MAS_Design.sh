...
config_path="./configs/X-MAS_Design_config.json"
...
for infer_name in "${infer_names[@]}"; do
  python X-MAS-Design/eval_mas.py --model_name llama-3.1-70b-instruct --model_config $config_path --dataset_names "${TEST_DATASET_NAMES[@]}" --infer_name $infer_name --eval_mode bench-test
done
