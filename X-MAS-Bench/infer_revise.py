...
def revise_init_answers(query, answer):
    # answers = [answers[i] for i in shuffle_index]

    str = "You are given a [Question] and a [Solution] to the question. Your task is to revise and provide a final complete solution to the question by reasoning over the given solution. The question and the solution are as follows:\n\n"
...
def get_sample_pool(test_dataset_name):
    ...
    with open(f"X-MAS-Bench/results/{test_dataset_name}/qwen2.5-32b-instruct/direct/qwen2.5-32b-instruct_direct.jsonl", "r") as f:
        for i, line in enumerate(f):
            ...
            sample_copy["revise_query"] = revise_init_answers(query, sample_copy["generated_output"])
